-- Retry migration with corrected policy SQL (remove stray quotes)
-- 1) Exams
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_exam_drafts_last_saved_at') THEN
    ...
    FOR EACH ROW EXECUTE FUNCTION public.set_exam_drafts_last_saved_at();
  END IF;
END $$;