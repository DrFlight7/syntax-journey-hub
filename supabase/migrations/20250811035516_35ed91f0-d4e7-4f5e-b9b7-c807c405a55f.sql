-- Re-run migration with correct dependency order
-- 1) Exams table
...
-- Teacher policy first (no cross-table deps)
DO $$ BEGIN
  IF NOT EXISTS (
...
-- 2) Exam assignments (needed for student select policy on exams)
...
-- Now add student policy on exams referencing exam_assignments
DO $$ BEGIN
  IF NOT EXISTS (
...
-- Trigger for updated_at on exam_tasks
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_exam_tasks_updated_at') THEN
...
-- Function and trigger to auto-update last_saved_at
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_exam_drafts_last_saved_at') THEN
